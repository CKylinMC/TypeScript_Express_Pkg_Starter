...
export function numberRerange(number, min, max) {
    return Math.min(Math.max(number, min), max);
}
export function datetimeFormater(format, date=new Date, utc=false) {
...
export function envBool(key, fallback = false){
    if (process.env[key]) {
        if(process.env[key].toLowerCase()=='yes') return true;
    }
    return !!fallback;
}
...
export function envInt(key, fallback = 0) {
    if (process.env[key]) {
        if(!isNaN(process.env[key] as unknown as number)) return +process.env[key];
    }
    return +fallback;
}

export function commonlize(obj):any {
    return JSON.parse(JSON.stringify(obj));
}

export function timeout(ms, msg = "Timed out", { errorclass } = {errorclass:Error}) {
    return wait(ms).then(_ => Promise.reject(new (errorclass ?? Error)(msg)));
}
